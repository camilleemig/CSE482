...
class MovieRatingData:
    class __MovieRatingData:
        ...
        def prep_movies_data(self):
            ...
            df_ratings_filtered, df_ratings_filtered_test = train_test_split(df_ratings_filtered, test_size=0.66, random_state=1)
            # df_movies_filtered, df_movies_filtered_test = train_test_split(df_movies_filtered, test_size=0.66, random_state=1)
            ...
            test_user_movie_mat = df_ratings_filtered_test.pivot(
                index='userId', columns='movieId', values='rating')
            ...
            for user in test_user_movie_mat.index:
                test_users_to_movie_ratings[user] = {}
                row = test_user_movie_mat.loc[user]
                for movie, rating in zip(test_user_movie_mat.columns, row):
                    if pd.isna(rating):
                        continue
                    test_users_to_movie_ratings[user][movieId_to_movieName[movie]] = rating
            ...
            self.TEST_DATA = test_users_to_movie_ratings
